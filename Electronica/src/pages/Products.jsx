...
const handleEditProduct = async (productId, updatedProductDetails) => {//Para editar el pruducto
  ...
  const handleImageChange = (event) => {//Convierte la imagen a base64
...
reader.onloadend=()=>{
  ...
  const handleFilterChange = (e) => {//Reconoce lo que hay en el modal
  ...
  const filterProducts = () => {//Filtro complnente
  ...
  const handleBuyProduct = (productId) => {//Si el usuario esta logueado puede comprar ,sino, no.
    if (!loggedIn) {
  ...
  const handleSubmitNewProduct = async (event) => {//Modal para agregar producto
